...
def save_data(tempdata, type, ticker, year, week):
    tempdata.drop(['AskOpen', 'AskHigh', 'AskLow', 'AskClose'], axis=1, inplace=True)
...
def download_data(ticker, year, week):
    ...
    try:
        if not is_file_present(ticker, 'M', year, week):
            tempdata = pd.read_csv(url_path_m, compression='gzip', storage_options=storage_options)
            save_data(tempdata, 'M', ticker, year, week)

        if not is_file_present(ticker, 'H', year, week):
            tempdata = pd.read_csv(url_path_h, compression='gzip', storage_options=storage_options)
